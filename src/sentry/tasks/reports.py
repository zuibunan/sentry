...
from sentry.models import Activity, Group, Project, ProjectStatus, Organization, OrganizationStatus, Team, User, GroupStatus
from sentry.tasks.base import instrumented_task
from sentry.utils.dates import floor_to_utc_day, to_timestamp, to_datetime
...
def merge_reports(target, other):
    return (
        ...
        merge_sequences(target[1], other[1]),
        merge_sequences(
            target[2],
            other[2],
            merge_issue_lists,
...
def change(current, basis):
    if basis is None:
        return None

    try:
        (current - basis) / basis
    except ZeroDivisionError:  # TODO: Need a better strategy around this, probably?
        return None


def rewrite_issue_list((count, issues)):
    instances = Group.objects.in_bulk([id for id, _ in issues])
...
def to_context(report):
    ...
    return {
        ...
        'comparisons': {
            'last_week': change(aggregates[-1], aggregates[-2]),  # this week, last week
            'last_month': change(
                aggregates[-1],
                average(aggregates) if all(v is not None for v in aggregates) else None,
            )
        },
        'issues': (
            ('New Issues', rewrite_issue_list(issue_lists[0])),
            ('Reintroduced Issues', rewrite_issue_list(issue_lists[1])),
            ('Most Frequently Seen Issues', rewrite_issue_list(issue_lists[2])),
        ),
    }
